...
pub fn main() -> Result<(), io::Error> {
    let filename = "assets/model/box/box.blend";

    println!("print_name_position");
    print_name_position(filename);

    println!("print_blend");
    print_blend(filename, "output.txt")?;

    println!("print read mesh");
    let obj = read_mesh(filename);
    println!("{:#?}", obj);
