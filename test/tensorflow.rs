...
fn main() -> TractResult<()> {
    // load the model
    let mut model =
        tract_tensorflow::tensorflow().model_for_path("vendor/tract/examples/tensorflow-mobilenet-v2/mobilenet_v2_1.4_224_frozen.pb")?;
    ...
    let image = image::open("vendor/tract/examples/tensorflow-mobilenet-v2/grace_hopper.jpg").unwrap().to_rgb();
    let resized =
